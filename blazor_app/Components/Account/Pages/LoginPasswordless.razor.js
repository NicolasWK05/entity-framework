window.webauthnLogin = async function (options) {
  ...
  if (publicKey.allowCredentials) {
    publicKey.allowCredentials.forEach((c) => {
...
window.finishPasskeyLogin = async function (payload) {
  // NOTE: payload.userName must now be set by the caller (Razor component) -
  // the server endpoint looks the user up by username, not email.
